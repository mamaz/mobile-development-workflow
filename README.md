...
### Release Candidate Flow - Development Phase and Maintenance Phase
...
**Notes**
	...
	- Both on Create Build step.

### Implementation

Since we use CircleCI, it is configured in `config.yml` file in `.circleci` folder on your iOS or Android project. **Checkout** and **Install Runner Dependencies** is written as a **run** step on `config.yml` while the implementation details of **Build Creation** and **Build Distribution** steps is written in `Fastfile` . 
...
and here’s the example of the `Fastfile` of `adhoc` lane:
